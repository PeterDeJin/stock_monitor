...
> Secret 設定後就算是 public repo 也看不到內容，這是安全注入到環境變數的標準做法。**金鑰絕對不要寫在程式碼裡**，任何 `git log` 都翻得到。
...
> 注意：手動觸發若在非交易時段（例如凌晨），程式會等到當天 13:35 才結束。
...
**Q：看到 ` 跳過問題合約: 'int' object is not an instance of 'str'`？**
