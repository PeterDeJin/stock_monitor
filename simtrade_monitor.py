...
def get_dynamic_market_list(api):
    ...
    # 抓 TPEx 上櫃「處置有價證券」名單（股票代號在 SecuritiesCompanyCode 欄位）
    try:
        res = requests.get("https://www.tpex.org.tw/openapi/v1/tpex_disposal_information",
                           headers=req_headers, timeout=10)
        for row in res.json():
            code = str(row.get("SecuritiesCompanyCode", "")).strip()
            if code:
                disposal_excluded.add(code)
        print(f"🚫 上櫃處置股名單：{len(disposal_excluded)} 檔")
    except Exception as e:
    ...
    print(f"📋 上櫃族群篩選後：{len(candidate_contracts)} 檔")
